...
Чтобы уточнить список проверки, можно использовать ключ `--to`, чтобы задать цели назначения. Например, чтобы проверить доступость к активам:
...
Пример иерархии фильтров событий:
```yaml
type: folder_node
